...
export class PinoLoggerInterceptor implements NestInterceptor {
  ...
  protected errorLog(context: ExecutionContext, err: any, methodLabel: string): void {
  ...
  private logHTTPResponse(
    ...
    if (level === 'error') {
      meta.http.status_code = responseBody?.statusCode ?? responseBody?.status ?? 500;
      meta.error =
  ...
  private constructHTTPLog(context: ExecutionContext): any {
    const req = context.switchToHttp().getRequest();
